...
import { parseSignalScrambleState } from "./signal-scramble-runtime-state";
...
export function SignalScramblePackagePage() {
  const { launchContext } = usePackageLaunchContext("signal-scramble");
    ...
    (tone: ActivityItem["tone"], summary: string, detail?: string) => {
      startTransition(() => {
        setActivity((current) => [
          {
            detail,
            id: createClientId("signal-scramble-activity"),
            summary,
  ...
  const handleRealtimeMessage = useEffectEvent((message: RealtimeServerMessage) => {
    switch (message.type) {
      ...
      case "game_state":
        ...
        appendActivity("success", "Signal board 已更新", `revision ${message.payload.revision}`);
        return;
      case "room_presence":
        appendActivity(
  ...
  useEffect(() => {
    ...
    socket.addEventListener("open", () => {
      ...
      appendActivity("success", "Signal Scramble 实时连接已建立", launchContext.roomId ?? "-");
      socket.send(
        JSON.stringify({
          message_id: createClientId("signal-room"),
      ...
      socket.send(
        JSON.stringify({
          message_id: createClientId("signal-state"),
          payload: { game_id: "signal-scramble", room_id: launchContext.roomId },
    ...
    socket.addEventListener("close", () => {
      ...
      setRoomStatus("idle");
      appendActivity("warn", "Signal Scramble 实时连接已关闭");
    });

    socket.addEventListener("error", () => {
      setRoomStatus("error");
      appendActivity("error", "Signal Scramble 实时连接发生错误");
    });
  ...
  const scrambleState =
    gameState?.gameId === "signal-scramble"
      ? parseSignalScrambleState(gameState)
      : null;
  const currentPlayerScore = scrambleState?.scores[launchContext.passengerId] ?? 0;
  const currentPlayerProgress = scrambleState?.progressByPlayer[launchContext.passengerId] ?? 0;
  const currentPlayerNextTarget =
    scrambleState?.nextTargetByPlayer[launchContext.passengerId] ?? null;
  const currentPlayerIsWinner =
    scrambleState?.winnerPlayerIds.includes(launchContext.passengerId) ?? false;
  const canActivateNode =
    roomStatus === "connected" &&
    !!activeRoom &&
    activeRoom.players.some((player) => player.player_id === launchContext.passengerId) &&
    !scrambleState?.isCompleted &&
    (scrambleState?.players.length ?? 0) >= 2;

  const rewardPoints = useMemo(() => {
    if (!scrambleState?.isCompleted) {
      return 0;
    }
  ...
  }, [currentPlayerIsWinner, currentPlayerScore, scrambleState]);

  useEffect(() => {
    if (!scrambleState?.isCompleted || !activeRoom) {
      return;
    }
    ...
    const reportSignature = [
      ...
      rewardPoints,
      scrambleState.winnerPlayerIds.join(","),
    ...
    void reportPoints({
      ...
      metadata: {
        ...
        completed_at: scrambleState.completedAtByPlayer[launchContext.passengerId],
        progress: currentPlayerProgress,
        target_count: scrambleState.targetSequence.length,
        winner_player_ids: scrambleState.winnerPlayerIds
      },
      passenger_id: launchContext.passengerId,
      points: rewardPoints,
      reason: currentPlayerIsWinner
        ? "signal scramble winner completed"
        : "signal scramble participant completed",
      report_id: [
        "signal-scramble",
        launchContext.passengerId,
        launchContext.sessionId,
  ...
  function handleActivateNode(nodeId: string) {
    ...
    socket.send(
      JSON.stringify({
        message_id: createClientId("signal-event"),
        payload: {
          gameId: room.game_id,
          payload: { nodeId },
  ...
  const statusLabel = (() => {
    ...
    if (scrambleState.isCompleted) {
      return scrambleState.winnerPlayerIds.length === 0 ? "本局结束" : "竞速已结束";
    }
    if (scrambleState.players.length < 2) {
      return "等待第二位乘客加入";
    }
    return currentPlayerNextTarget
      ? `下一目标 ${currentPlayerNextTarget}`
      : "等待下一目标";
  })();
    ...
    <main className="package-shell">
      <section className="package-hero">
        <div>
          ...
          <h1>Signal Scramble</h1>
          <p className="lede">
            双人异步竞速激活 relay 节点。双方都在同一个 signal board 上推进自己的序列，
            先完整走通目标链路的乘客获胜。
          </p>
        </div>
        <div className="hero-stats">
          ...
          <article className="stat-chip accent-mint">
            <span>Progress</span>
            <strong>
              {currentPlayerProgress}/{scrambleState?.targetSequence.length ?? 0}
            </strong>
          </article>
      ...
      <section className="package-grid">
        <article className="panel">
          ...
          <div className="json-card">
            <p className="mini-label">target sequence</p>
            <pre>{JSON.stringify(scrambleState?.targetSequence ?? [], null, 2)}</pre>
          </div>
        </article>
        ...
        <article className="panel panel-span-2">
          <div className="panel-heading">
            <div>
              <p className="panel-kicker">Async Race</p>
              <h2>Signal Board</h2>
            </div>
            <div className="activity-topline">
              <span>双方可并发点击</span>
              <span>错误节点不会推进进度</span>
            </div>
          </div>

          <div className="signal-layout">
            <div className="signal-grid">
              {(scrambleState?.signalNodes ?? []).map((node) => {
                const currentPlayerActivated =
                  scrambleState?.activatedNodeIdsByPlayer[launchContext.passengerId]?.includes(
                    node.id
                  ) ?? false;
                const isNextTarget = currentPlayerNextTarget === node.id;

                return (
                  <button
                    className={`signal-node accent-${node.accent}${currentPlayerActivated ? " is-activated" : ""}${isNextTarget ? " is-target" : ""}`}
                    disabled={!canActivateNode}
                    key={node.id}
            ...
            <div className="signal-sidebar">
              <div className="scoreboard">
                {Object.entries(scrambleState?.scores ?? {}).map(([playerId, score]) => (
                  <article className="score-row" key={playerId}>
                    <strong>{playerId}</strong>
                    <span>
                      {score} pts
                      {scrambleState?.winnerPlayerIds.includes(playerId) ? " · 胜者" : ""}
                    </span>
                  </article>
                ))}
              </div>
